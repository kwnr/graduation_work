...
while cap.isOpened():
            ...
            if hull is not None:
                ...
                if defects is not None:
                    if len(defects)==2:
                        ...
                        cv2.putText(img, f'AREA: {cv2.contourArea(contours[i])}, COORD: ({cx}, {cy})', tuple(contours[i][0][0]),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2)
                        ...
                        head_p=21-np.sum(points[:,0]+1)-np.sum(points.reshape(4))
    ...
    cv2.putText(merged,f'{fps:.2f}',[10,50],cv2.FONT_HERSHEY_SIMPLEX,2,(0,255,0),3)       
