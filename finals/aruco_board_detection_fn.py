...
class Detector:
    ...
    def solve_pnp(self, objp, imgp):
        retval, rvec, tvec = cv2.solvePnP(
            objp, imgp, self.camera_matrix, self.dist_coeffs, flags=cv2.SOLVEPNP_IPPE
        )
        # retval,rvec,tvec,inliers=cv2.solvePnPRansac(objp,imgp,self.camera_matrix,self.dist_coeffs,flags=cv2.SOLVEPNP_IPPE)
        rvec, tvec = cv2.solvePnPRefineLM(
            objp, imgp, self.camera_matrix, self.dist_coeffs, rvec, tvec
        )
        return rvec, tvec
    ...
    def run(self, draw=False):
        ...
        if self.objp is not None:
            ...
            if draw:
                ...
                cv2.drawFrameAxes(
                    img, self.camera_matrix, self.dist_coeffs, self.rvec, self.tvec, 100
                )
                cv2.putText(
                    img,
                    f"{self.tvec}",
